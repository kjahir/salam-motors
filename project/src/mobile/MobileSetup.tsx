...
import { Bell, ChevronRight, History as HistoryIcon, ScrollText, ShieldCheck, UserCog, Users, Warehouse, Wallet } from "lucide-react";
...
/**
 * Everything the desktop sidebar reaches that the four bottom-nav tabs do not. Each row
 * opens the same page component the desktop renders (wrapped in MobileDesktopPage), so
 ...
 */
const SETUP_ITEMS: { screen: MobileScreen; page: PageKey; labelKey: string; icon: ReactNode }[] = [
  { screen: "inventory", page: "inventory", labelKey: "nav.inventory", icon: <Warehouse size={18} /> },
...
export function MobileSetup({ onNavigate }: { onNavigate: MobileNavigate }) {
  const { t } = useTranslation();
  const { canAccessPage } = usePermissions();
  const items = SETUP_ITEMS.filter((item) => canAccessPage(item.page));
  ...
  return (
    <div>
      <div className="bg-mobile-navy text-white px-5 pt-6 pb-8">
        ...
        <h1 className="font-poppins text-2xl font-bold mt-1">{t("mobileSetup.title")}</h1>
        <p className="text-xs text-white/70 mt-1">{t("mobileSetup.description")}</p>
      </div>
